...
/* ids_compare_destip is a short function to sort by destination IP. 
 *
 * This function returns 
 *    -1 if rec_a should be closer to the front than rec_b,
 *     1 if rec_b is to be considered closer to the front, and 
 *     0 if the records are equal for sorting.
 *
 * For the alert data we want to sort from highest IP down, so
 * closer to the front means a larger IP.
 *
 * The function expects pointers to two record structures, and it is an error
 * if either is NULL
 */
...
/* adds new element to queue end
 *
 * inputs: 
 *    list_ptr: a pointer to either sorted or unsorted list
 *
 *    id: generator ID for the new element
 * 
 *    ip: destination IP for the new element
 *
 * output: appended list
 */

void ids_append_rear(llist_t *list_ptr, int id, int ip){
    alert_t *new_ptr = (alert_t *)calloc(1, sizeof(alert_t));
    new_ptr->dest_ip_addr = ip;
    new_ptr->generator_id = id;
...
/* sorts the queue by generator id in ascending order
 *
 * inputs: 
 *    list_ptr: a pointer to either sorted or unsorted list
 *
 *    type: number indicating how it is to be sorted
 * 
 * output: sorted list
 */
void ids_sort_gen(llist_t *list_ptr, int type){
...
/* sorts the queue by destination ip in descending order
 *
 * inputs: 
 *    list_ptr: a pointer to either sorted or unsorted list
 *
 ...
 * output: sorted list
 */
void ids_sort_ip(llist_t *list_ptr, int type){
    ...
    if(list_ptr!=NULL&&list_ptr->ll_front!=NULL){
        ...
        int initialcount = llist_entries(list_ptr);
        start = clock();
        llist_sort(list_ptr, type, ids_compare_destip);
        end = clock();
        elapse_time = 1000.0 * ((double)(end-start))/ CLOCKS_PER_SEC;
        assert(llist_entries(list_ptr) == initialcount);
        printf("%d\t%f\t%d\n", initialcount, elapse_time, type);
