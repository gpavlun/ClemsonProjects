...
int main(int argc, char *argv[])
{
    ...
    char junk[MAXLINE];
    int command_number1;   // number found with command
    int command_number2;
    int token_count;
    ...
    //printf("Queue      : ADDREAR; RMFRONT; SORTGEN x; SORTIP x; APPENDREAR generator_id dest_ip_addr; PRINTQ\n");
    //printf("           : STATS; QUIT\n");
    ...
    while (fgets(line, MAXLINE, stdin) != NULL) {
        token_count = sscanf(line, "%s%d%d%s", command, &command_number1, &command_number2, junk);
        if (token_count == 1 && strcmp(command, "ALERT") == 0) {
            ...
            ids_list_gen(sorted_list, command_number1);

        } else if (token_count == 2 && strcmp(command, "LISTIP") == 0) {

            ids_list_ip(sorted_list, command_number1);

        } else if (token_count == 2 && strcmp(command, "DELGEN") == 0) {

            ids_remove_gen(sorted_list, command_number1);

        } else if (token_count == 2 && strcmp(command, "DELIP") == 0) {

            ids_remove_ip(sorted_list, command_number1);

        } else if (token_count == 2 && strcmp(command, "SCANGEN") == 0) {

            ids_scan(sorted_list, command_number1);
        ...
        } else if (token_count == 2 && strcmp(command, "SORTGEN") == 0) {

            ids_sort_gen(unsorted_list, command_number1);

        } else if (token_count == 2 && strcmp(command, "SORTIP") == 0) {

            ids_sort_ip(unsorted_list, command_number1);

        } else if (token_count == 3 && strcmp(command, "APPENDREAR") == 0) {

            ids_append_rear(unsorted_list, command_number1, command_number2);
