 ...
 * keys in the table using table_search.  Use
 *   -r run the retrieve driver and specifiy the type of initial table keys 
 *      with -i {rand|seq}.  rand is the default.
...
static int Seed = 12012025;

#define CLASSSIZE 40
char *classroll[] = {
"dalber4", "warnold", "tbannon", "jrbowma", "dbrook5", "sheltob",
"ignacic", "dcoope3", "jrdnl"  , "oestola", "cfenton", "aferna6",
"jflinte", "enzog"  , "jrgardn", "jcgasqu", "lgellar", "khubert",
"zadeh"  , "eajarre", "cpj4"   , "makoon" , "gnlee"  , "mlutsuk",
"clmccau", "pmegee" , "snm5"   , "ammrrs" , "snallam", "asp6"   ,
"gpavlun", "jross6" , "mrude"  , "egschaf", "jskapin", "underw4",
"lveenst", "jiwilde", "mbjett" , "harlanr",
};
...
int main(int argc, char **argv)
{
    ...
    hashes_configure(HashAlg);  // defaults to djb
    printf("Seed: %d\n", Seed);
    srand48(Seed);
...
#define TABLESIZE 7
    ...
    printf("\n----- Deletion driver for table size 7. You must compare to HW9.6-----\n");
    if (ProbeDec != LINEAR) {
    ...
    for (i = 0; i < num_del; i++) {
        printf("key = %s\n",keys[del[i]]);
        table_debug_print(H);
        int *rp = table_search(H, keys[del[i]]);
...
#define DFD_KEYS 9
    ...
    printf("\n----- Deletion driver for full table with size 7. You must compare to HW9.7 --\n");
    // generate keys for locations.  One less than table size
    for (i = 0; i < DFD_KEYS; i++) {
        do {
            char *key = generate_random_id();
            key[0] = '0' + i;   // makes keys unique and readable
            if (hashes_table_pos(key, TABLESIZE) == locations[i]) {
                keys[i] = key;
                break;
            }
...
void RehashDriver(int test_M)
{
        ...
        *ip = 10*i;
        if (Verbose) {
            printf("%d: about to insert %s at position %d", i, classroll[i], 
                    hashes_table_pos(classroll[i], test_M));
            if (ProbeDec == DOUBLE) {
                printf(" with decrement %d", hashes_probe_dec(classroll[i], test_M));
            }
            printf("\n");
        }
        assert(table_full(H) == 0);
    ...
    assert(NULL == table_search(H, classroll[i]));
    ...
    ip = table_search(H, classroll[1]);  // check key is not there
    assert(ip == NULL);
    ...
    if (Verbose) {
        printf("\n about to insert %s at %d\n", classroll[CLASSSIZE-1], 
                hashes_table_pos(classroll[CLASSSIZE-1], test_M));
    }
    ...
    *ip = 123;
    ...
    ip = NULL;
    ip = table_search(H, classroll[CLASSSIZE-1]);
    assert(*(int *)ip == 123);
    ip = NULL;
    assert(table_entries(H) == test_M-1);
    ...
    if (Verbose) {
        ...
        table_debug_print(H);
        printf("\n about to insert 'abcdefg' at %d\n", hashes_table_pos("abcdefg", test_M));
    }
    ...
    *ip = 76;
    ...
    ip = table_search(H, classroll[CLASSSIZE-1]);
    assert(*(int *)ip == 123);
    ip = table_search(H, classroll[2]);
    assert(*(int *)ip == 77);
    ip = table_search(H, classroll[3]);
    assert(*(int *)ip == 87);
    ip = table_search(H, classroll[0]);
    assert(*(int *)ip == 76);
    ip = table_search(H, "abcdefg");
    assert(*(int *)ip == 456);
    ip = NULL;
    // rehash
    //printf("\nnew size = %d\n",test_M);
    table_rehash(H, test_M);
    if (Verbose) {
        printf("table after rehash with 5 items\n");
        table_debug_print(H);
    }
    assert(table_entries(H) == 5);
    assert(table_deletekeys(H) == 0);
...
void RetrieveDriver()
{
    ...
    if (Trials > 0) {
        ...
        for (i = 0; i < Trials; i++) {
            ...
            if (Verbose) {
                printf("%d: looking for %s at %d", i, key, hashes_table_pos(key, TableSize));
                if (ProbeDec == DOUBLE) {
                    printf(" with decrement %d", hashes_probe_dec(key, TableSize));
                }
                printf("\n");
            }
            dp = table_search(test_table, key);
            if (dp == NULL) {
    ...
    for (i = 0; i < CLASSSIZE; i++) {
        if (Verbose && SUPERVERBOSE) {
            printf("%d: looking for %s at %d", i, classroll[i], 
                    hashes_table_pos(classroll[i], TableSize));
            if (ProbeDec == DOUBLE) {
                printf(" with decrement %d", hashes_probe_dec(classroll[i], TableSize));
            }
            printf("\n");
        }
        dp = table_search(test_table, classroll[i]);
        if (dp == NULL) {
            unsuc_search += table_stats(test_table);
            unsuc_trials++;
            if (Verbose && SUPERVERBOSE)
...
void equilibriumDriver(void)
{
    ...
    if (ProbeDec == CHAIN && LoadFactor > 0.5  && LoadFactor < 1.5)
    {
        ...
        /* The design of the equilibrium driver depends on the uniform
    ...
    printf("Found %d of the class keys. Did not find %d that were removed\n", suc_trials, unsuc_trials);

    /* rehash and retest table */
    printf("  Rehash table\n");
    table_rehash(test_table, TableSize);
    ...
    if (Verbose)
        printf("Start looking for class keys\n");
    int suc_trials_rehashed, unsuc_trials_rehashed;
    suc_search = suc_trials_rehashed = unsuc_search = unsuc_trials_rehashed = 0;
    for (i = 0; i < CLASSSIZE; i++) {
        /* random key */
        if (Verbose) {
            printf("%d: looking for %s at %d", i, classroll[i], 
    ...
    printf("After rehash, found %d of the class keys. Did not find %d\n", suc_trials_rehashed, unsuc_trials_rehashed);
    if (suc_trials != suc_trials_rehashed || unsuc_trials != unsuc_trials_rehashed) {
        printf("\n\tThe rehash is broken!!!\n");
        printf("There were %d class keys in table before rehash, and %d after\n",
                suc_trials, suc_trials_rehashed);
        printf("\t not in table before %d and after %d\n", 
                unsuc_trials, unsuc_trials_rehashed);
        exit(24);
    }
...
int build_random(table_t *T, int table_size, int num_addr)
{
    ...
    for (i = 0; i < num_addr; i++) {
        ...
        *ip = i;
        if (Verbose) {
            printf("%d: %s, h: %d", i, key, hashes_table_pos(key, table_size));
            if (ProbeDec == DOUBLE) {
                printf(" d: %d", hashes_probe_dec(key, table_size));
            }
            printf("\n");
        }
...
void getCommandLine(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "m:a:h:f:i:t:s:erbdv")) != -1)
        switch(c) {
